...
namespace HREngine.Bots
{
    public class Settings
    {
        ...
        public bool playaround; //play around some enemys aoe-spells
        ...
        public int twotsamount; // number of boards where the second AI step is simulated
        ...
        public int berserkIfCanFinishNextTour; // 0 - off, 1 - if there is any chance to kill the enemy through the round, all attacks will be in the face

        public int alpha = 50; // weight of the second turn in calculation (0<= alpha <= 100)
        public bool useSecretsPlayAround; // playing arround enemys secrets
        public int placement; // 0 - minions are interleaved by value (..low value - hi value..), 1 - hi val minions along the edges, low val in the center

        //use this only with useExternalProcess = true !!!!
        public bool useExternalProcess; // // use external process for calculations
        public bool passiveWaiting; // process will wait passive for external process to finish calculations

        public int ImprovedCalculations = 1; // 0 - disabled(for old PCs), 1 - enabled
        public int speedupLevel; // 0 - normal speed (the safest - wait animation), 1 - speed up face attacks, 2 - 1 + speed up attack on minions
        public int adjustActions; // test!! - reorder actions after calculations: 0 - as calculated (by Default), 1 - AoE first
        public int printRules; //0 - off, 1 - on

        public int concedeMode; //0 - concede if direct defeat, 1 - concede if Playfield has a value <= -10000
        //###########################################################
        ...
        public void setSettings(string behavName, bool nameIsPath = false)
        {
            ...
            if (!nameIsPath)
            {
                ...
                {
                    Helpfunctions.Instance.ErrorLog($"{behavName}: 应用自定义配置.");
                }
                else
                {
                    pathToSettings = Path.Combine(Silverfish.Instance.BehaviorPath[behavName], "_settings.txt");
                }
            }
            ...
            try
            {
                ...
                foreach (var s in lines)
                {
                    ...
                    {
                        case "enfacehp":
                            this.enfacehp = valueInt;
                            break;
                        case "weaponOnlyAttackMobsUntilEnfacehp":
                            this.weaponOnlyAttackMobsUntilEnfacehp = valueInt;
                            break;
                        case "maxwide":
                            this.maxwide = valueInt;
                            break;
                        case "playaround":
                            this.playaround = valueBool;
                            break;
                        case "playaroundprob":
                            this.playaroundprob = valueInt;
                            break;
                        case "playaroundprob2":
                            this.playaroundprob2 = valueInt;
                            break;
                        case "twotsamount":
                            this.twotsamount = valueInt;
                            break;
                        case "enemyTurnMaxWide":
                            this.enemyTurnMaxWide = valueInt;
                            break;
                        case "enemyTurnMaxWideSecondStep":
                            this.enemyTurnMaxWideSecondStep = valueInt;
                            break;
                        case "berserkIfCanFinishNextTour":
                            this.berserkIfCanFinishNextTour = valueInt;
                            break;
                        case "concedeMode":
                            this.concedeMode = valueInt;
                            break;
                        case "printRules":
                            this.printRules = valueInt;
                            break;
                        case "nextTurnDeep":
                            this.nextTurnDeep = valueInt;
                            break;
                        case "nextTurnMaxWide":
                            this.nextTurnMaxWide = valueInt;
                            break;
                        case "nextTurnTotalBoards":
                            this.nextTurnTotalBoards = valueInt;
                            break;
                        case "useSecretsPlayAround":
                            this.useSecretsPlayAround = valueBool;
                            break;
                        case "alpha":
                            this.alpha = valueInt;
                            break;
                        case "placement":
                            this.placement = valueInt;
                            break;
                        case "useExternalProcess":
                            this.useExternalProcess = valueBool;
                            break;
                        case "passiveWaiting":
                            this.passiveWaiting = valueBool;
                            break;
                        case "ImprovedCalculations":
                            this.ImprovedCalculations = valueInt;
                            break;
                        case "speedupLevel":
                            this.speedupLevel = valueInt;
                            break;
                        case "adjustActions":
                            this.adjustActions = valueInt;
                            break;
                        default:
        ...
        private void endOfSetSettings()
        {
            ...
            if (this.twotsamount > 0)
            {
                Helpfunctions.Instance.ErrorLog("推算下回合敌方行动");
            }

            if (this.useSecretsPlayAround)
            {
                Helpfunctions.Instance.ErrorLog("开启防奥秘");
            }
            ...
            if (this.writeToSingleFile)
            {
                Helpfunctions.Instance.ErrorLog("write log to single file");
            }
        }
