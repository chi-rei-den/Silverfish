        ...
        private bool checkCondition(Condition cond, Playfield p, Action a = null)
        {
            ...
            switch (cond.parameter)
            {
                ...
                case param.ohc_minions_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Type == CardType.MINION) tmp_counter++;
                ...
                case param.ohc_spells_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Type == CardType.SPELL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_secrets_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Secret) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_weapons_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Type == CardType.WEAPON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_murlocs_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.MURLOC) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_demons_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.DEMON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_mechs_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.MECHANICAL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_beasts_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.PET) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_totems_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.TOTEM) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_pirates_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.PIRATE) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_Dragons_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.DRAGON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_elems_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Race == Race.ELEMENTAL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_shields_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.DivineShield) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.ohc_taunts_equal:
                    tmp_counter = 0;
                    foreach (Handcard hc in p.owncards) if (hc.card.Taunt) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_murlocs_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.MURLOC) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_murlocs_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.MURLOC) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_demons_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.DEMON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_demons_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.DEMON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_mechs_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.MECHANICAL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_mechs_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.MECHANICAL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_beasts_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.PET) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_beasts_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.PET) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_totems_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.TOTEM) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_totems_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.TOTEM) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_pirates_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.PIRATE) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_pirates_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.PIRATE) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_Dragons_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.DRAGON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_Dragons_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.DRAGON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.omc_elems_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if (m.handcard.card.Race == Race.ELEMENTAL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.emc_elems_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if (m.handcard.card.Race == Race.ELEMENTAL) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                ...
                case param.enboard_notcontain:
                    ...
                    return true;
                case param.ownhand_contain:
                    foreach (Handcard hc in p.owncards) if (hc.card.CardId == cond.cardID) return true;
                    return false;
                case param.ownhand_notcontain:
                    foreach (Handcard hc in p.owncards) if (hc.card.CardId == cond.cardID) return false;
                    return true;
